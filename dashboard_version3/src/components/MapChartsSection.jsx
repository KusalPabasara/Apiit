...
function MapChartsSection({ incidents }) {
  ...
  const typeData = [
    ...
    { 
      name: 'Road Block', 
      value: incidents.filter(i => i.incident_type === 'road_block').length, 
      color: '#dc2626',
      icon: '🚧'
    },
    { 
      name: 'Power Line', 
      value: incidents.filter(i => i.incident_type === 'power_line_down').length, 
      color: '#7c3aed',
      icon: '⚡'
    }
  ].filter(d => d.value > 0);
          ...
          <div className="mt-2 grid grid-cols-2 gap-1">
            {typeData.map((type, i) => (
              <div key={i} className="flex items-center gap-1">
                ...
                <span className="text-[10px] text-gray-600">{type.value}</span>
              </div>
            ))}
          </div>
        </div>
