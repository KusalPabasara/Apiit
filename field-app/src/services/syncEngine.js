...
import { getUnsyncedReports, markReportSynced, db } from '../db/database';
...
class SyncEngine {
  ...
  async syncAll(silent = false) {
    ...
    try {
      // Get all unsynced items from all tables (excluding SOS - handled separately)
      const [unsyncedReports, unsyncedTrapped, unsyncedBlocked, unsyncedSupply] = await Promise.all([
        getUnsyncedReports(),
      ...
      const totalUnsynced = unsyncedReports.length + 
      ...
      if (totalUnsynced > 0 || !silent) {
        console.log(`📤 Found ${totalUnsynced} unsynced items (${unsyncedReports.length} incidents, ${unsyncedTrapped.length} trapped, ${unsyncedBlocked.length} blocked roads, ${unsyncedSupply.length} supply)`);
      }
      ...
      // Note: SOS alerts are handled separately in HomePage.jsx with direct fetch
      // They are not synced through the sync engine to ensure fastest possible response
  ...
  // Note: SOS alerts are handled separately in HomePage.jsx with direct fetch
  // They bypass the sync engine to ensure fastest possible response time
  ...
  async getStatus() {
    ...
    // Note: SOS alerts are not included - they are handled separately
    const totalPending = unsyncedReports.length + 
                        unsyncedTrapped.length + unsyncedBlocked.length + unsyncedSupply.length;
