...
export default function IncidentReportForm() {
  ...
  // Form state
  const [types, setTypes] = useState([]); // Changed to array for multiple selections
  const [severity, setSeverity] = useState(3);
  const [description, setDescription] = useState('');
  const [photo, setPhoto] = useState(null);
  const [voiceNote, setVoiceNote] = useState(null); // Base64 audio data
  ...
  // Store report ID to track sync status
  ...
  // Listen for sync complete events to update UI when report syncs
  useEffect(() => {
    if (!submitted || !savedOffline || !currentReportIdRef.current) return;
    
    const handleSyncComplete = async (data) => {
      // Check if our report was synced
      if (currentReportIdRef.current) {
        const report = await db.reports.get(currentReportIdRef.current);
        if (report && report.syncStatus === 1) {
          console.log('✅ UI updated: Report synced successfully');
          setSavedOffline(false);
        }
      }
    };
    
    // Subscribe to sync events and store the wrapped callback
    const wrappedCallback = syncEngine.subscribe('syncComplete', handleSyncComplete);
    
    // Cleanup
    return () => {
      if (wrappedCallback) {
        syncEngine.unsubscribe('syncComplete', wrappedCallback);
      }
    };
  ...
  const startRecording = async () => {
    try {
      ...
      // Use webm for smaller file size, fallback to mp4
      ...
      recordingTimerRef.current = setInterval(() => {
        ...
        console.log('⏱️ Recording:', newDuration + 's');
  ...
  const handleSubmit = async (e) => {
    ...
    const report = {
      id: reportId,
      localId: reportId,
      type: types, // Store as array for multiple types
      types: types, // Also store as 'types' for clarity
    ...
    try {
      ...
      // Store report ID for sync tracking
      ...
      if (isOnline) {
        try {
          ...
          // Normalize types to lowercase for backend
          const normalizedTypes = report.types.map(t => t.toLowerCase().replace(/\s+/g, '_'));
          console.log('📤 Sending incident with types:', normalizedTypes); // Debug log
          
          const payload = {
            local_id: report.id,
            incident_type: normalizedTypes, // Send array of types
            incident_types: normalizedTypes, // Also send as 'incident_types' for clarity
          ...
          const response = await fetch(`${getApiUrl()}/incidents/device`, {
            method: 'POST',
            headers: {
  ...
  return (
    <div className="min-h-screen safe-top safe-bottom">
      {/* Header */}
      <header className="sticky top-0 z-[9999] bg-slate-900 flex items-center gap-3 p-4 border-b border-slate-700 shadow-lg">
        <button
          onClick={() => navigate(-1)}
      ...
      <form onSubmit={handleSubmit} className="p-4 space-y-5">
        ...
        {/* Incident Type - REQUIRED (Multiple Selection) */}
        <div>
          <label className="block text-sm font-medium text-gray-400 mb-2">
            Incident Type <span className="text-red-400">*</span>
            {types.length > 0 && (
              <span className="text-xs text-gray-500 ml-2">({types.length} selected)</span>
            )}
          </label>
          <div className="grid grid-cols-3 gap-2">
            {INCIDENT_TYPES.map((incidentType) => {
              const isSelected = types.includes(incidentType.value);
              return (
                <button
                  ...
                  onClick={() => {
                    if (isSelected) {
                      setTypes(types.filter(t => t !== incidentType.value));
                    } else {
                      setTypes([...types, incidentType.value]);
                    }
                  }}
                  className={`p-3 rounded-xl border-2 text-left transition-all ${
                    isSelected
                      ? `border-${incidentType.color.replace('#', '')} bg-${incidentType.color.replace('#', '')}/20`
                      : 'border-slate-700 bg-slate-800 hover:border-slate-600'
                  }`}
                  style={isSelected ? {
                    borderColor: incidentType.color,
                    backgroundColor: `${incidentType.color}20`
                  } : {}}
                >
                  <span className="text-2xl mb-1 block">{incidentType.icon}</span>
                  <span className="text-sm font-medium">{incidentType.label}</span>
                  {isSelected && (
                    <span className="text-xs text-emerald-400 mt-1 block">✓ Selected</span>
                  )}
                </button>
              );
            })}
          ...
          {types.length === 0 && (
            <p className="text-xs text-amber-400 mt-1">Please select at least one incident type</p>
          )}
        </div>
        ...
        <div>
          ...
          {photo ? (
            ...
            <label className="flex flex-col items-center justify-center h-24 border-2 border-dashed border-slate-700 rounded-xl cursor-pointer hover:border-slate-600 transition-colors">
          ...
          {voiceNote ? (
            ...
            <div className="flex items-center gap-3 p-3 bg-slate-800 rounded-xl border border-slate-700">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-gray-400">{recordingDuration}s</p>
              </div>
              <button
                type="button"
            ...
            <div className="flex items-center justify-center">
              {isRecording ? (
                <div className="flex items-center gap-4 p-3 bg-red-500/20 rounded-xl border border-red-500/30 w-full">
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-3 border-2 border-dashed border-slate-700 rounded-xl hover:border-slate-600 transition-colors w-full justify-center"
                >
                  <Mic className="w-5 h-5 text-gray-500" />
                  <span className="text-sm text-gray-500">Tap to record voice message</span>
                </button>
              )}
            </div>
          )}
        ...
        <button
          type="submit"
          disabled={submitting || types.length === 0 || !location}
