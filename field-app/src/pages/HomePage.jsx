...
function HomePage() {
  ...
  const triggerSos = async () => {
    ...
    try {
      ...
      // FAST PATH: Send SOS immediately with short timeout, no retries
      // This ensures dashboard gets the alert as fast as possible while maintaining atomicity
      if (isOnline) {
        const apiUrl = getApiUrl();
        console.log('🆘 FAST PATH: Sending SOS to:', `${apiUrl}/sos`);
        
        // Use fast timeout (3 seconds) and no retries for urgent SOS
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), 3000); // 3 second timeout for SOS
        ...
        try {
          const response = await fetch(`${apiUrl}/sos`, {
            ...
            body: JSON.stringify({
              id: sosData.id, // Include ID for idempotency
              local_id: sosData.id,
              device_id: sosData.deviceId,
          ...
          if (response.ok || response.status === 200) {
            ...
            // ✅ ATOMICITY: Only mark as synced after confirmed server success
            try {
              await db.emergencies.update(sosData.id, { syncStatus: 1 });
              console.log('✅ SOS Alert sent FAST and marked as synced!', result);
            } catch (updateError) {
            ...
            // Keep as unsynced, will retry via sync engine later
          }
        } catch (fetchError) {
          clearTimeout(timeoutId);
          if (fetchError.name === 'AbortError') {
            console.warn('⏱️ SOS request timeout (3s) - will retry via sync engine');
          } else {
            console.error('❌ Network error sending SOS:', fetchError.message);
          }
          // Keep as unsynced, will retry via sync engine later
        }
      } else {
