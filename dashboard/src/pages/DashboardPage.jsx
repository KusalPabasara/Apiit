...
function DashboardPage({ onLogout }) {
  ...
  const getTypeIcon = (type) => {
    const icons = { landslide: '⛰️', flood: '🌊' };
    return icons[type] || '⚠️';
  };

  const getTypeName = (type) => {
    const names = { landslide: 'Landslide', flood: 'Flood' };
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900">
      ...
      <main className="pt-16 lg:pt-14">
        {loading && incidents.length === 0 ? (
          ...
          <>
            {/* Map View with 3-Column Layout */}
            {activeTab === 'map' && (
              <div className="h-[calc(100vh-3.5rem)] lg:h-[calc(100vh-3.5rem)] flex">
                {/* Left Sidebar - KPIs (Desktop Only) */}
                ...
                <div className="flex-1 relative flex flex-col">
                  ...
                  {/* Bottom KPIs Overlay */}
                  <div className="absolute bottom-4 left-4 right-4 xl:left-4 xl:right-4 z-10">
                    <MapBottomKPIs incidents={incidents} />
                  </div>
                </div>

                {/* Right Panel - Charts (Desktop Only) */}
                <div className="hidden xl:block w-96 bg-gray-50 border-l border-gray-200 p-3 overflow-y-auto">
